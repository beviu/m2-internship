...
level 0 (the privilege level of the OS) and jumps to the handler associated with
page faults (`#PF`) in the Interrupt Descriptor Table. The handler comes from
the OS and so the application cannot directly control page fault handling.
...
+ The application initializes the userfaultfd FD with the `UFFDIO_API` ioctl,
  ...
  mode_. We will focus on the signal notitication mode here.
...
- Transfer control to user (interface for the app to receive the kernel metadata
  from a VMA such as the PFNs so that the application can reuse them when
  enabling UFs)
...
When the ring buffer is full, the sender generates a fault and traps in the
kernel so that it can schedule it out. The kernel notifies the handler that it
needs to handle a fault and tell the kernel when it's done so that it can
schedule in the sender again.

For the case where the handler is schedule out, we can use a mechanism similar
to uintr_wait, where the kernel changes the NV field in the UPID of the handler
so that trying to send a user interrupt to the handler thread will result in a
trap in the kernel.

We can reuse the UITT, as if it was a user interrupt. The receiver knows whether
a user interrupt comes from a user fault or a `senduipi` by looking at the
vector number.
...
- the handler is a different thread from the faulting thread (use user
  interrupt + ring buffer)

What about nesting? In the self-UF case, do we disable user faults before
jumping to the handler and renable them when we return from the handler? Do we
do something like user interrupt where the CPU pops RFLAGS during UIRET so
software that wants user interrupts disabled after UIRET has to modify the
pushed RFLAGS.

We would have structure that points to the ring buffer and also contains
metadata such as the head and tail indices. They would both be shared between
the faulting and handling thread. They could be aligned to a cacheline.
