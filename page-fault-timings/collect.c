...
static void print_first_row() {
  ...
  fputs("c_exit,iret,end,msg_received", stdout);
...
int main(int argc, char **argv) {
  ...
  uint32_t timestamp_iret_low;
  uint32_t timestamp_iret_high;
  uint32_t timestamp_c_exit_low;
  uint32_t timestamp_c_exit_high;
  ...
  for (i = 0; i < 100000; ++i) {
    ...
    if (access == PROT_READ) {
      asm volatile("movb (%[byte]), %[byte_copy]"
                   : "=&a"(timestamp_iret_low), "=&d"(timestamp_iret_high),
                     "=&D"(timestamp_c_exit_low),
                     "=&S"(timestamp_c_exit_high), [byte_copy] "=r"(byte_copy)
                   : [byte] "r"(byte), "b"(PF_TRACING_MAGIC)
                   : "ecx");
    } else {
      asm volatile("movb %[byte_copy], (%[byte])"
                   : "=&a"(timestamp_iret_low), "=&d"(timestamp_iret_high),
                     "=&D"(timestamp_c_exit_low), "=&S"(timestamp_c_exit_high)
    ...
    timestamp_iret = timestamp_iret_low | ((uint64_t)timestamp_iret_high << 32);
    timestamp_c_exit =
        timestamp_c_exit_low | ((uint64_t)timestamp_c_exit_high << 32);
