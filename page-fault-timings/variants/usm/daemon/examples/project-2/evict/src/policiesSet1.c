...
#ifdef SpEMUL
int swap_out_emul(struct to_swap *victimNode) {
#ifdef SpDEBUG
  printf("[swapDevPols/sp_out/] X\n");
#endif
  ...
  if (usm_clear_pfn(
          victimNode->proc, victimNode->swapped_address,
          swap_value(victimNode))) { // some clearance first..? Of 3 bits...
#ifdef SpDEBUG
    printf("[swapDevPols/sp_out/Mayday] Clr-emul. failed!\n");
    getchar();
#endif
  ...
  /* Replaced by latency emulator.. */
  // if
  // (unlikely(fwrite((void*)(victimNode->page->data),SYS_PAGE_SIZE/*ToBeComplxfd*/,1,(FILE
  // *)victimNode->swapDevice->backend)!=1)) {
  /*#ifdef SpDEBUG
                  perror("[swapDevPols/sp_out/Mayday] Swap out writing
  failed!\n"); getchar(); #endif
          if(usm_set_pfn_emul(victimNode->proc,victimNode->swapped_address,victimNode->page->physicalAddress,
  0)) getchar(); restore_used_page(&((struct optEludeList
  *)(victimNode->page->usedListPositionPointer))->iulist);
  ...
  // commented for emulation purposes.. hence ain't used again.. so close to
  // double thingy (consumed mem.) per launch/bench

  // list_add(&((struct optEludeList
  // *)(victimNode->page->usedListPositionPointer))->iulist,&freeList);
  // //list_move_tail(&((struct optEludeList
  // *)(victimNode->page->usedListPositionPointer))->iulist,&freeList); // some
  // unneeded list_del.. maybe some verification after the pages' putting...
  usedMemory -= SYS_PAGE_SIZE;
  pthread_mutex_unlock(&policiesSet1Ulock);
  return 0;
}

int swap_in_emul(struct to_swap *luckyNode) {
...
SEEK_SET)) { #ifdef SpDEBUG perror("[swapDevPols/sp_in/Mayday] Fseeko
failed\n"); getchar(); #endif
      // yeah.. put back in here, makes more sense.. and better/best avoids
different threads trying to swap the same entries.. let's delete from start. |
yup, TODO.. analogy to swap_out's double thingy.. return 1;
  }
  LIST_HEAD(pageToSwapInsideOf);
  if (!get_pages(&pageToSwapInsideOf,1))
      return 1;
  struct optEludeList * chosenPage = list_first_entry(&pageToSwapInsideOf,struct
optEludeList, iulist); if
(unlikely(fread((void*)chosenPage->usmPage->data,SYS_PAGE_SIZE,1,(FILE
*)luckyNode->swapDevice->backend)!=1)) { #ifdef SpDEBUG
...
get ordered back later... but still, this all simply comes from an error too...
      pthread_mutex_lock(&policiesSet1Swaplock);
      list_add(&luckyNode->globlist,&swapList);         // __
      pthread_mutex_unlock(&policiesSet1Swaplock);
      return 1;
  }
  usmSetUsedListNode(chosenPage->usmPage,chosenPage);       //there my man! And
push this all to basic_swap..! Now everyone needs to to it themselves!
  ...
  // locks but meh, super temporary..
  pthread_mutex_lock(&policiesSet1Ulock);
  usedMemory += SYS_PAGE_SIZE;
  pthread_mutex_unlock(&policiesSet1Ulock);

  /* Reminder : this needed for ahead swapping.. but conflicting with
   * basic_swap*_in*.. dk dis state but understandable. */
  // getchar();
...
int basic_swap(
  ...
  printf("Swapping in! | swap_value: %lu ^^' | normed %lu \n", event->offst,
         (event->offst << 5 >> (5 + 9)));
#endif
  /*struct to_swap *neededNode = event_to_swap_entry(event);        // emul..
  if(!neededNode) {
      printf("Come on man.. : %lu | %lu\n", event->offst >> (sizeof(unsigned
  long)*8-5), (event->offst << 5 >> (5+9))/4096); getchar();
  }*/
  /*pthread_t culler;
  pthread_create(&culler, NULL, delete_swap_entry, neededNode);*/   // rid this off (in swap_in..)... TODO
  if (swap_in(NULL)) { // emul.  //
                       // if(unlikely(neededNode->swapDevice->swap_dev_ops.swap_in(neededNode)))
                       // {        // return this directly... if not for maybe
                       // what's inside..
  ...
  // event->paddr=neededNode->page->physicalAddress;  // emul. : same page
  // there's brought up back
  if (usmSubmitSwapEventEmul(event)) {
#ifdef SpDEBUG
    printf("[devPolicies/swp/Mayday] Unapplied allocation\n");
    getchar();
#endif
    // put_pages(&((struct optEludeList
    // *)(neededNode->page->usedListPositionPointer))->iulist);    // emul..
    // usmAddProcSwapNode(luckyNode);              // fails ordering.... might
    // get ordered back later... but still, this all simply comes from an error
    // too...
    pthread_mutex_lock(&policiesSet1Swaplock);
    // list_add(&neededNode->globlist,&swapList);         // __     // emul : no
    // actual swap list..
    pthread_mutex_unlock(&policiesSet1Swaplock);
    return 1;
  }
  ...
  restore_used_page(&((struct optEludeList
  *)(neededNode->page->usedListPositionPointer))->iulist);     //  name to be
  changed..
  ...
  // luckyNode->page->usedListPositionPointer=..   // shouldn't ever change
  actually putSwapNode(neededNode->swapDevice,&neededNode->snode->iulist);
  free(neededNode);*/
  return 0;
}
#else
...
void handle_evict_request(
  ...
  hold_used_page(&victimPageNode->iulist);     // pick_and_hold_used_page.. TODO

  if (usmCheckPageAlive(victimPageNode->usmPage)) {
    pthread_mutex_lock(&policiesSet1Flock);
    list_add(&(victimPageNode->iulist),
             &freeList); // list_move_tail(&((struct optEludeList
                         // *)(victimNode->page->usedListPositionPointer))->iulist,&freeList);
                         // // some unneeded list_del.. maybe some verification
                         // after the pages' putting...
    // list_add(&((struct optEludeList
    // *)(victimPageNode->usmPage->usedListPositionPointer))->iulist,&freeList);
    // //list_move_tail(&((struct optEludeList
    // *)(victimNode->page->usedListPositionPointer))->iulist,&freeList); //
    // some unneeded list_del.. maybe some verification after the pages'
    // putting...
    usedMemory -= SYS_PAGE_SIZE;
    pthread_mutex_unlock(&policiesSet1Flock);
    printf("[devPol/Oof] Lingering free page taken back! %.2f%s\n",
           usedMemory / 1024 / 1024 > 0 ? (float)usedMemory / 1024 / 1024
                                        : (float)usedMemory / 1024,
           usedMemory / 1024 / 1024 > 0 ? "MB" : "KB");
    return;
  }
...
out:
  ...
  /*if(usedMemory>=1*1024*1024) {
      ...
      // victimPageNode =
list_first_entry(&procDescList[event->origin].usedList,struct optEludeList,
proclist); victimPageNode = list_first_entry(&usedList, struct optEludeList,
iulist); hold_used_page(&victimPageNode->iulist);
      // pthread_mutex_unlock(&policiesSet1Ulock);
#ifdef SpDEBUG
          printf("\tS'ping out!\n");
#endif
      struct to_swap *toSwapNode= (struct to_swap *)malloc(sizeof(struct
to_swap));           // some wasted memory during emulation.. but meh.. if
(toSwapNode == NULL) { printf("\tDude!..\n"); getchar();
      }
      toSwapNode->page=victimPageNode->usmPage;
      toSwapNode->proc=victimPageNode->usmPage->process;
      if(toSwapNode->proc<=0) {
...
#else
struct usm_swap_dev_ops swap_device_emul_ops = {.swap_in = swap_in_emul,
                                                .swap_out = swap_out_emul};
struct usm_swap_dev swap_device_emul = {.number = 0};
#endif

struct usm_swap_dev *
numberToSwapDev(int n) { // a devices' table        // TODO complete..
#ifndef SpEMUL
  return &swap_device_one;
#else
  return &swap_device_emul;
#endif
}

struct usm_swap_dev *pick_swap_device(int proc) { // TODO complete dis.
                                                  // if usedMemory&co
...
#else
  swap_device_emul.backend = fopen(swapFileName, "wb+");
  if (!swap_device_emul.backend) {
#ifdef SpDEBUG
    perror("Swap file opening failed!");
#endif
    return 1;
  }
  if (init_swap_device_nodes(&swap_device_emul,
                             (unsigned long)4 * 1024 * 1024 * 1024)) {
#ifdef SpDEBUG
    printf("Swap device's nodes couldn't be init'ed!\n");
#endif
...
#else
  fclose((FILE *)swap_device_emul.backend);
#endif
}
