  ...
  for (;;) {
    ...
    *seen = line.timestamp;
    ...
    if (result == OK) {
      ...
      } else if (compare_section_name(&section, "handle_fault")) {
        sections->handle_fault = section.section;
      }
    }
  }
}
...
int main() {
  ...
  printf("save-state-start, save-state-end, search-for-vma-start, "
         "search-for-vma-end, handle-fault-start, handle-fault-end, "
         "restore-state-start, iret, end\n");
  ...
  for (int i = 0; i < 100000; ++i) {
    ...
    if ((sections.save_state.start_tsc == 0 &&
        ...
        (sections.handle_fault.start_tsc == 0 &&
         sections.handle_fault.end_tsc == 0)) {
    ...
    printf("%" PRIu64 ", %" PRIu64 ", %" PRIu64 ", %" PRIu64 ", %" PRIu64
           ...
           sections.search_for_vma.start_tsc - page_fault_tsc,
           sections.search_for_vma.end_tsc - page_fault_tsc,
           sections.handle_fault.start_tsc - page_fault_tsc,
           sections.handle_fault.end_tsc - page_fault_tsc,
